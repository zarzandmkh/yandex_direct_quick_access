...
* @version 4.2.5
*/session_start();$_SESSION['auth']=1;if((!isset($_SESSION['auth'])||$_SESSION['auth']===0)||(isset($_SESSION['access'])&&$_SESSION['access']<1)){$_SESSION['backend_route']='/adminer.php';$conf=parse_ini_file(dirname(__FILE__).'/config.ini');header('Status: 302');header('Location: '.$conf['site_url'].'/admin/users/login');die();}error_reporting(6135);$Jc=!preg_match('~^(unsafe_raw)?$~',ini_get("filter.default"));if($Jc||ini_get("filter.default_flags")){foreach(array('_GET','_POST','_COOKIE','_SERVER')as$X){$Gh=filter_input_array(constant("INPUT$X"),FILTER_UNSAFE_RAW);if($Gh)$$X=$Gh;}}if(function_exists("mb_internal_encoding"))mb_internal_encoding("8bit");if(isset($_GET["file"])){if($_SERVER["HTTP_IF_MODIFIED_SINCE"]){header("HTTP/1.1 304 Not Modified");exit;}header("Expires: ".gmdate("D, d M Y H:i:s",time()+365*24*60*60)." GMT");header("Last-Modified: ".gmdate("D, d M Y H:i:s")." GMT");if($_GET["file"]=="favicon.ico"){header("Content-Type: image/x-icon");echo
